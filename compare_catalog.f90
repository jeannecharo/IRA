program compare_catalog
  ...
  ! INTEGER, ALLOCATABLE  :: idx_all(:,:)       ! Id in the catalog that match the local struct of analyze structure
  LOGICAL       :: fail
  ! REAL, ALLOCATABLE   :: dist_all(:,:)
    ...
    IF (analyze(istruc)%typ(iat) /= 999) then
     ...
     ! ... Add the atom information in the output if it matches the patterns in catalog
      IF (dist_lowest <= 5) THEN
       ...
       DO jat = 2, n_list
        IF (analyze(istruc)%typ(list(jat)) /= 999) THEN
...
subroutine compare_Q( nat_ref, typ_ref_in, coords_ref_in, &
  ...
  call cshda( nat_ref, typ_ref, coords_ref, &
   nat_loc, typ_loc, coords_loc, &
   found(1:nat_loc), dists(1:nat_loc) )
